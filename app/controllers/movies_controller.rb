class MoviesController < ApplicationController
 ...
 def index
     ...
     if params[:title_sort] == 'title' #if the title header was clicked
     	...
     	session[:release_date_sort] = nil #release date cannot also be clicked, clear session
     	...
     	session[:title_sort] = nil #title cannot also be clicked, clear session

     end  
     
     if (session[:title_sort] != nil) && (session[:release_date_sort] == nil)  
     	@movies = Movie.where(:rating=>@checked_ratings).order(session[:title_sort])	
     
     end
     if (session[:release_date_sort] != nil) && (session[:title_sort] == nil)
